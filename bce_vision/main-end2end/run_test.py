...
def test(args, use_modal, local_rank):
    if use_modal:
        if 'resnet' in args.CV_model_load:
            ...
            cv_model_load = '../../pretrained_models/swin/' + args.CV_model_load
    ...
    run_eval_test(model, item_id_to_keys, args.lmdb_data, users_history_for_test,
                  users_test, 256, item_num, use_modal, args.mode, local_rank)


def run_eval_test(model, item_id_to_keys, lmdb_path, user_history, users_eval,
                  batch_size, item_num, use_modal, mode, local_rank):
