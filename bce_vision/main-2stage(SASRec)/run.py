...
from model import Model, Resnet_Encoder, Swin_Encoder
...
def train(args, local_rank):
    ...
    if 'resnet' in args.CV_model_load:
        ...
        cv_encoder = Swin_Encoder(image_net=cv_model).to(local_rank)
    ...
    train_dataset = BuildTrainDataset(u2seq=users_train, item_content=item_image_embs, item_num=item_num,
        ...
        if not need_break and now_epoch%1==0:
            ...
            max_eval_value, max_epoch, early_stop_epoch, early_stop_count, need_break, need_save = \
                run_eval(now_epoch, max_epoch, early_stop_epoch, max_eval_value, early_stop_count,
                         model, item_image_embs, users_history_for_valid, users_valid, 256, item_num,
...
def run_eval(now_epoch, max_epoch, early_stop_epoch, max_eval_value, early_stop_count,
             model, item_image_embs, user_history, users_eval, batch_size, item_num,
