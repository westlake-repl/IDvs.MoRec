...
from .dataset import BuildEvalDataset, Build_Lmdb_Eval_Dataset, SequentialDistributedSampler
...
def get_itemId_embeddings(model, item_num, test_batch_size, args, local_rank):
    model.eval()
    item_dataset = ItersDataset(data=np.arange(item_num + 1))
...
def eval_model(model, user_history, eval_pairs, user_embeddings, item_embeddings, test_batch_size,
    ...
    eval_dataset = BuildEvalDataset(eval_pairs=eval_pairs, user_embs=user_embeddings, item_num=item_num)
