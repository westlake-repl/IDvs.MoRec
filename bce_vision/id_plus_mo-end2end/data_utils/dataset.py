...
class Build_Id_Dataset(Dataset):
    def __init__(self, u2seq, item_num, max_seq_len):
        self.u2seq = u2seq
    ...
    def __getitem__(self, user_id):
        ...
        neg_items = [0] * mask_len + neg_items + [0]
        sample_items.append(neg_items)
        sample_items = torch.LongTensor(sample_items).transpose(0, 1)

        return sample_items, sample_items, torch.FloatTensor(log_mask)


class Build_Lmdb_Dataset(Dataset):
    def __init__(self, u2seq, item_num, max_seq_len,
                 db_path, item_id_to_keys, resize):
        self.u2seq = u2seq
        self.item_num = item_num
        self.max_seq_len = max_seq_len + 1
    ...
    def __getitem__(self, user_id):
        ...
        sample_items_id = []
        padding_seq_id = [0] * mask_len + seq
        sample_items_id.append(padding_seq_id)
        neg_items_id = []
        ...
        with env.begin() as txn:
            for i in range(tokens_Len):
                ...
                sample_items[0][mask_len + i] = image_trans
                # neg
                sam_neg = random.randint(1, self.item_num)
                while sam_neg in seq:
                    sam_neg = random.randint(1, self.item_num)
                neg_items_id.append(sam_neg)
                byteflow = txn.get(self.item_id_to_keys[sam_neg])
                IMAGE = pickle.loads(byteflow)
                sample_items[1][mask_len + i] = self.transform(Image.fromarray(IMAGE.get_image()).convert('RGB'))
            # target
            IMAGE = pickle.loads(txn.get(self.item_id_to_keys[seq[-1]]))
            image_trans = self.transform(Image.fromarray(IMAGE.get_image()).convert('RGB'))
            sample_items[0][mask_len + tokens_Len] = image_trans

        neg_items_id = [0] * mask_len + neg_items_id + [0]
        sample_items_id.append(neg_items_id)
        sample_items_id = torch.LongTensor(sample_items_id).transpose(0, 1)

        sample_items_content = torch.FloatTensor(sample_items).transpose(0, 1)

        return sample_items_id, sample_items_content, torch.FloatTensor(log_mask)
