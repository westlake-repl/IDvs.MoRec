...
def get_itemLMDB_embeddings(model, item_num, item_id_to_keys,
                            lmdb_data, test_batch_size, args, local_rank):
    model.eval()
    item_dataset = Build_Lmdb_Eval_Dataset(data=np.arange(item_num + 1),
                                           item_id_to_keys=item_id_to_keys,
                                           db_path=os.path.join(args.root_data_dir, args.dataset,
                                                                lmdb_data),
    ...
    with torch.no_grad():
        for input_ids, input_contents in item_dataloader:
            ...
            input_embs_content = model.module.cv_encoder(input_contents)
