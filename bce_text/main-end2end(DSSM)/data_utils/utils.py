...
def setuplogger(dir_label, log_paras, time_run, mode, rank, behaviors):
    log_code = None
    ...
    if rank in [-1, 0]:
        if 'train' in mode:
            log_path = os.path.join('./logs_' + dir_label + '_' + 'train')
            if not os.path.exists(log_path):
                os.makedirs(log_path)
            log_file_name = os.path.join(log_path, 'log_' + log_paras + time_run + '.log')
        elif 'test' in mode:
            behaviors = behaviors.split('_')[0]
            log_file_name = 'log_test_all_' + behaviors + '.log'
        elif 'special' in mode:
            behaviors = behaviors.split('_')[0]
            log_file_name = 'log_special_' + behaviors + '.log'
        else:
            behaviors = behaviors.split('_')[0]
            log_file_name = 'log_other_' + behaviors + '.log'
...
def save_model(now_epoch, model, model_dir, optimizer, rng_state, cuda_rng_state, save_label, Log_file):
    ckpt_path = os.path.join(model_dir, f'epoch-{now_epoch}-{save_label}.pt')
