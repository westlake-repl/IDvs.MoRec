...
def test(args, use_modal, local_rank):
    ...
    if 'special' in args.mode:
        seqs_for_cold, history_for_cold, seqs_for_new, history_for_new, new_item_id_to_keys = \
            read_behaviors_special(os.path.join(args.root_data_dir, args.dataset, args.cold_seqs),
                                   os.path.join(args.root_data_dir, args.dataset, args.new_seqs),
                                   os.path.join(args.root_data_dir, args.dataset, args.new_items),
                                   item_name_to_id, Log_file, use_modal)
    ...
    if 'special' in args.mode:
        run_eval_special(model, item_id_to_keys, new_item_id_to_keys,
                         args.lmdb_data, args.new_lmdb_data, 256, item_num, use_modal,
                         seqs_for_cold, history_for_cold, seqs_for_new, history_for_new, local_rank)


def run_eval_special(model, item_id_to_keys, new_item_id_to_keys,
                     lmdb_path, new_lmdb_path, batch_size, item_num, use_modal,
                     seqs_for_cold, history_for_cold, seqs_for_new, history_for_new, local_rank):
    eval_start_time = time.time()
    if use_modal:
        new_item_num = len(new_item_id_to_keys)
        item_embeddings = get_itemLMDB_embeddings(model, item_num, item_id_to_keys, lmdb_path, batch_size, args,
                                                  local_rank)
        new_item_embeddings = get_itemLMDB_embeddings(model, new_item_num, new_item_id_to_keys, new_lmdb_path, batch_size, args, local_rank)
    else:
        item_embeddings = get_itemId_embeddings(model, item_num, batch_size, args, local_rank)
        new_item_embeddings = None
    eval_model_special(model, item_embeddings, new_item_embeddings, batch_size, args, item_num, Log_file,
                       seqs_for_cold, history_for_cold, seqs_for_new, history_for_new, local_rank, use_modal)
